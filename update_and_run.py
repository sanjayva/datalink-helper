...
#########################################################
#
#  Datalink API Framework
#
#  Author: Sanjay Valiyaveettil
#
#
#########################################################
...
APIPUBLICKEY = ''
APISECRETKEY = ''
BASEURL4DATALINK = 'https://dl-apptiotr-tbmc-manager.apptio.com'
agent_id = ''
...
def get_connector_config(connectorid):
...
def get_connector_status(connectorid):
    ...
    connector_config_resp = requests.get(connector_url, headers={"Content-Type": "application/json", "apptio-opentoken": auth_token})
    if connector_config_resp.status_code == 200:
        return connector_config_resp.json()['statusMessage']
    else:
...
def prettyprint(jsontext):
    """ Function to format json output in human readable format """
